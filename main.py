...
# ফায়ারবেস ইনিশিয়াল সেটিংস (লিঙ্কটি একদম সঠিকভাবে বসানো আছে)
cred = credentials.Certificate("firebase-adminsdk.json")
firebase_admin.initialize_app(cred, {'databaseURL': 'https://telegram-mini-bot-5cb21-default-rtdb.asia-southeast1.firebasedatabase.app/'})
...
# --- স্টার্ট ও ওয়ান-টোন মিনিমাল লাক্সারি মেসেজ ---
@dp.message_handler(commands=['start'])
async def start(message: types.Message):
    uid = message.from_user.id
    args = message.get_args()
    
    # রেফারেল ট্র্যাকিং ও ব্যালেন্স ডিস্ট্রিবিউশন
    ref = db.reference(f'users/{uid}')
...
async def web_app_data_receive(message: types.Message, state: FSMContext):
    ...
    # ১. ডিপোজিট রিকোয়েস্ট ভেরিফিকেশন ৩-স্টেপ
    ...
    # ২. লাইভ সার্ভিস রিকোয়েস্ট লোডার
    ...
    # ৪. কোডিং ফাইল পার্সেস ও ডেলিভারি সিস্টেম
        ...
        if srv_ref and u_data:
            ...
            if current_bal >= price:
                ...
                # ফাইল সাথে সাথে ইউজারকে ইনবক্সে পাঠিয়ে দেওয়া
...
# --- স্ক্রিনশট প্রসেসিং এবং হিস্ট্রি জেনারেশন ---
...
async def process_ss(message: types.Message, state: FSMContext):
    ...
    # ফায়ারবেসে পেন্ডিং স্ট্যাটাস ট্র্যাকিং রেকর্ড অ্যাড
...
# --- অ্যাডমিন এডিট এবং অ্যাকশন কলব্যাকস ---
...
async def admin_callbacks(call: types.CallbackQuery, state: FSMContext):
    ...
    # নাম্বার ও লোগো পরিবর্তন সাব-মেনু
...
# --- ডাইনামিক ডাটা প্রসেসিং হ্যান্ডলারস (FSM) ---
